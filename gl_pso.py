...
class Particle:
    ...
    def update_velocity(self, pos_best_g, global_best_error, pbest_avg):
        eps = 1e-12

        for i in range(len(self.position_i)):

            # GLBest equations
            w = 1.1 - (global_best_error / (pbest_avg + eps))
            c = 1.0 + (global_best_error / (self.err_best_i + eps))
            ...
            r = random()
            self.velocity_i[i] = w * self.velocity_i[i] + c * r * (pos_best_g[i] + self.pos_best_i[i] - 2 * self.position_i[i])
...
def minimize(costFunc, verbose=False):
    ...
    while i < MAXITER:
        ...
        pbest_avg = np.mean([swarm[j].err_best_i for j in range(NUM_PARTICLES)])

        # cycle through swarm and update velocities and position
        for j in range(NUM_PARTICLES):
            swarm[j].update_velocity(pos_best_g, err_best_g, pbest_avg)
